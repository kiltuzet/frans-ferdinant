...
if __name__ == "__main__":
    ...
    def deleteDriver(self):
        ...
        self.conn.commit()
        self.loadDrivers()
        self.log_action(f"Удалён водитель ID={driver_id}")

    def deleteBus(self):
        row = self.bus_table.currentRow()
        if row == -1:
            QMessageBox.warning(self, "Ошибка", "Выберите автобус")
            return
        bus_id = self.bus_table.item(row, 0).text()
        self.cursor.execute("DELETE FROM buses WHERE id=?", (bus_id,))
        self.conn.commit()
        self.loadBuses()
        self.log_action(f"Удалён автобус ID={bus_id}")
    ...
    def deleteRoute(self):
        ...
        self.conn.commit()
        self.loadRoutes()
        self.log_action(f"Удалён маршрут ID={route_id}")
    ...
    def deleteMaintenance(self):
        ...
        if row == -1:
            ...
            return
        maint_id = self.maint_table.item(row, 0).text()
        self.cursor.execute("DELETE FROM maintenance WHERE id=?", (maint_id,))
        self.conn.commit()
        self.loadMaintenance()
        self.log_action(f"Удалено ТО ID={maint_id}")
